...
class ChargesCalculator:
    ...
    def calculate_charges(self):
        ...
        for coord, charge in dimorphite_charges.items():
            charged_atom = kdtree.search(coord, 0.1, level="A")

            if len(charged_atom) > 1:
                print(charged_atom)
                exit("Atoms too close!")

            charged_atom[0].pdb2pqr_charge += charge
    ...
    def protonate_heteroresidues(self):
        ...
        for atom in structure.get_atoms():
            atom.protonated_by_dimorphite = False
            atom.dimorphite_charge = 0
            ...
            for CCD_mol_sdf in open("CCD/Components-pub.sdf", "r").read().split("$$$$\n"):
                ...
                if mol_name in residues_processed_by_hydride_names:
                    ...
                    if CCD_mol is None:
                        ...
                        atom_indices_map = [x[1] for x in sorted(list(zip(dimorphite_mol.GetSubstructMatch(res.queryMol),
                        ...
                        if len(dimorphite_mol.GetAtoms()) != len(atom_indices_map):
                            ...
                            combined_charges = []
                            for CCD_atom, CCD_formal_charge, dimorphite_formal_charge in zip(CCD_mol.GetAtoms(), CCD_mol_formal_charges, dimorphite_formal_charges):
                                if CCD_formal_charge != 0:
                                    ...
                                    if all([atom.GetFormalCharge() == 0 for atom in bonded_atoms + bonded_atoms_over_two_bonds]):  # CCD_atom is already in bonded_atoms_over_two_bonds
        ...
        for res in residues_processed_by_hydride:
            ...
            for atom, formal_charge in zip(res.get_atoms(), formal_charges):
                atom.protonated_by_dimorphite = True
                atom.dimorphite_charge = formal_charge
                if formal_charge != 0:
                    self.hydride_charges[tuple(atom.coord)] = formal_charge
        ...
        protein = biotite_mmCIF.get_structure(self.biotite_mmCIF_file,
                                              model=1,
                                              include_bonds=True)
        protein.charge = [atom.dimorphite_charge for atom in structure.get_atoms()]
        ...
        protein_with_hydrogens, _ = hydride.add_hydrogen(protein, mask=[atom.protonated_by_dimorphite for atom in structure.get_atoms()])
        ...
        one_symbol_chains = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
        # TODO dodělat exit, pokud je chainů více, uvidíme co to udělá, zkusit háčky a čárky atd...
        # dopsat rekonverzi po zpracování
        presented_label_asym_id = {key: one_symbol_chains[index] for index, key in enumerate(sorted(set(self.biotite_mmCIF_file.block["atom_site"]["label_asym_id"].as_array())))}
        presented_auth_asym_id = {key: one_symbol_chains[index] for index, key in enumerate(sorted(set(self.biotite_mmCIF_file.block["atom_site"]["auth_asym_id"].as_array())))}
        self.biotite_mmCIF_file.block["atom_site"]["label_asym_id"] = [presented_label_asym_id[key] for key in self.biotite_mmCIF_file.block["atom_site"]["label_asym_id"].as_array()]
        self.biotite_mmCIF_file.block["atom_site"]["auth_asym_id"] = [presented_auth_asym_id[key] for key in self.biotite_mmCIF_file.block["atom_site"]["auth_asym_id"].as_array()]
    ...
    def protonate_protein(self):
        ...
        self.pdb2pqr_charges = prepared_molecule.charge
        print("ok")


if __name__ == "__main__":
    args = load_arguments()
    calculator = ChargesCalculator(args.mmCIF_file, args.data_dir, args.pH, args.delete_auxiliary_files)
    calculator.fix_structure()
    calculator.remove_hydrogens()
    calculator.protonate_heteroresidues()
    calculator.protonate_protein()
